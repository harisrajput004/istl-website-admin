...
export class AboutComponent {
  ...
  ngOnInit(): void {
    this.cmsService.getSectionsByPageId(2).subscribe({
      next: async (response) => {
        ...
        if (this.sections[2]?.backgroundImagePath) {
          this.originalBannerImage = await this.urlToFile(
            this.sections[2].backgroundImagePath,
            'banner.jpg'
          );
        }
      },
  ...
  onSubmit(): void {
    ...
    this.cmsService.updateSections(this.pageId, this.sections).subscribe({
      next: (response) => {
        ...
        if (bannerToSend || firstSectionToSend || secondSectionToSend) {
          this.cmsService.updateAboutSectionsImages(formData).subscribe({
